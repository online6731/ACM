...
void make_table() {
    for (int i = 0; i < heigth; i++)
        for (int j = 0; j < width; j++)
            if (table[i][j] == '.')
                table[i][j] = '0' +
                            (0 <= i - 1 && i - 1 < heigth && 0 <= j - 1 && j - 1 < width ? table[i - 1][j - 1] == '*' : 0) +
                            (0 <= i - 1 && i - 1 < heigth && 0 <= j && j < width ? table[i - 1][j] == '*' : 0) +
                            (0 <= i - 1 && i - 1 < heigth && 0 <= j + 1 && j + 1 < width ? table[i - 1][j + 1] == '*' : 0) +
                            (0 <= i && i < heigth && 0 <= j - 1 && j - 1 < width ? table[i][j - 1] == '*' : 0) +
                            (0 <= i && i < heigth && 0 <= j + 1 && j + 1 < width ? table[i][j + 1] == '*' : 0) +
                            (0 <= i + 1 && i + 1 < heigth && 0 <= j - 1 && j - 1 < width ? table[i + 1][j - 1] == '*' : 0) +
                            (0 <= i + 1 && i + 1 < heigth && 0 <= j && j < width ? table[i + 1][j] == '*' : 0) +
                            (0 <= i + 1 && i + 1 < heigth && 0 <= j + 1 && j + 1 < width ? table[i + 1][j + 1] == '*' : 0);
...
int main(){
    int step = 1;
    while (cin >> heigth >> width) {
        if (heigth + width == 0) break;
        for (int i = 0; i < heigth; i++) for (int j = 0; j < width; j++) cin >> table[i][j];
        make_table();
        cout << "Field #" << step << ":" << endl;
        for (int i = 0; i < heigth; i++)
            for (int j = 0; j < width; j++)
                cout << table[i][j] << (j == width - 1 ? "\n" : "");
        step++;
    }
    return 0;
}
